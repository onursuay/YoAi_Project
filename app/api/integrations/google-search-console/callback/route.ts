...
export async function GET(request: Request) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/${locale}/entegrasyon?gsc=error&reason=${encodeURIComponent(error)}`, origin),
      { status: 302 }
    )
  }
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL(`/${locale}/entegrasyon?gsc=error&reason=missing_code_or_state`, origin),
      { status: 302 }
    )
  }
  ...
  if (!expectedState || expectedState !== stateValue) {
    return NextResponse.redirect(
      new URL(`/${locale}/entegrasyon?gsc=error&reason=invalid_state`, origin),
      { status: 302 }
    )
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL(`/${locale}/entegrasyon?gsc=error&reason=${encodeURIComponent(reason)}`, origin),
      { status: 302 }
    )
  }
}
