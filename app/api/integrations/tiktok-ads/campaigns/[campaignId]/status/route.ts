...
/**
 * POST /api/integrations/tiktok-ads/campaigns/[campaignId]/status
 * Enable, disable (pause), or delete a TikTok Ads campaign.
 */
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { ok: true, campaignId: cid, status },
      { headers: { 'Cache-Control': 'no-store' } }
    )
  } catch (err) {
    console.error('[TikTok Campaign Status] Error:', err)
