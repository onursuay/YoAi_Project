...
/**
 * POST /api/integrations/tiktok-ads/adgroups/[adGroupId]/status
 * Enable, disable (pause), or delete a TikTok Ads ad group.
 */
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { ok: true, adGroupId: agid, status },
      { headers: { 'Cache-Control': 'no-store' } }
    )
  } catch (err) {
    console.error('[TikTok AdGroup Status] Error:', err)
