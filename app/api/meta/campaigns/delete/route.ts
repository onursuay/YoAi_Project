...
export async function POST(request: Request) {
  try {
    ...
    if (!campaignId || typeof campaignId !== 'string') {
      return NextResponse.json(
        { ok: false, error: 'invalid_input', message: 'campaignId is required' },
        { status: 400 }
      )
    }

    // Delete campaign via Meta Graph API.
    // Meta cascades: all ad sets and ads under the campaign are deleted automatically.
    const result = await metaClient.client.delete(`/${campaignId}`)
      ...
      return NextResponse.json(
        {
          ...
          error: metaError?.type === 'RateLimit' ? 'rate_limit_exceeded' :
                 ...
                 'meta_api_error',
          message: metaError?.message || 'Failed to delete campaign',
          meta: {
            code: metaError?.code,
