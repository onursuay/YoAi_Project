...
export async function POST(request: Request) {
  try {
    ...
    if (!campaignId || !name) {
      return NextResponse.json(
        { ok: false, error: 'invalid_input', message: 'campaignId and name are required' },
        { status: 400 }
      )
    }
    ...
    if (trimmedName.length > 256) {
      return NextResponse.json(
        { ok: false, error: 'validation_error', message: 'Campaign name is too long (max 256 characters)' },
        { status: 400 }
      )
    }
      ...
      return NextResponse.json(
        {
          ...
          error: result.error?.type === 'RateLimit' ? 'rate_limit_exceeded' :
                 ...
                 'meta_api_error',
          message: result.error?.message || 'Failed to rename campaign',
