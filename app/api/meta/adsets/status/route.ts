...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('[Ad Set Status Update] Request:', {
        adSetId,
        status,
    ...
    if (!adSetId || !status) {
      return NextResponse.json(
        { ok: false, error: 'invalid_input', message: 'adSetId and status are required' },
        { status: 400 }
      )
    }
    ...
    const updateResult = await metaClient.client.postForm(`/${adSetId}`, formData)
      ...
      return NextResponse.json(
        {
          ...
          message: metaError?.message || 'Failed to update ad set status',
        },
        { status: statusCode }
      )
    }

    const verifyResult = await metaClient.client.get(`/${adSetId}`, {
      ...
      return NextResponse.json(
        {
          ...
          message: `Status update failed: requested ${status}, but ad set is ${actualStatus}`,
