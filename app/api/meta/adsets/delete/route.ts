...
export async function POST(request: Request) {
  try {
    ...
    if (!adsetId || typeof adsetId !== 'string') {
      return NextResponse.json(
        { ok: false, error: 'invalid_input', message: 'adsetId is required' },
        { status: 400 }
      )
    }

    // Delete ad set via Meta Graph API.
    // Meta cascades: all ads under the ad set are deleted automatically.
    const result = await metaClient.client.delete(`/${adsetId}`)
      ...
      return NextResponse.json(
        {
          ...
          error: metaError?.type === 'RateLimit' ? 'rate_limit_exceeded' :
                 ...
                 'meta_api_error',
          message: metaError?.message || 'Failed to delete ad set',
          meta: {
            code: metaError?.code,
    ...
    return NextResponse.json({ ok: true, adsetId }, {
