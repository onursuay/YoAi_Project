...
export async function POST(request: Request) {
  try {
    ...
    const { adId, name } = body

    if (!adId || !name) {
      return NextResponse.json(
        { ok: false, error: 'invalid_input', message: 'adId and name are required' },
        { status: 400 }
      )
    }
    ...
    if (trimmedName.length < 1) {
      return NextResponse.json(
        { ok: false, error: 'validation_error', message: 'Ad name cannot be empty' },
        { status: 400 }
      )
    }
    ...
    if (trimmedName.length > 256) {
      return NextResponse.json(
        { ok: false, error: 'validation_error', message: 'Ad name is too long (max 256 characters)' },
        { status: 400 }
      )
    }
    ...
    const result = await metaClient.client.postForm(`/${adId}`, formData)
      ...
      return NextResponse.json(
        {
          ...
          error: result.error?.type === 'RateLimit' ? 'rate_limit_exceeded' :
                 ...
                 'meta_api_error',
          message: result.error?.message || 'Failed to rename ad',
    ...
    if (DEBUG) console.error('Ad rename error:', error)
    return NextResponse.json(
      {
        ok: false,
        error: 'server_error',
