...
export default function CommandCenterHeader({ health, lastAnalysis, loading, aiGenerated }: Props) {
  const formattedTime = lastAnalysis
    ? new Date(lastAnalysis).toLocaleString('tr-TR', {
        day: '2-digit',
        month: 'short',
        year: 'numeric',
        hour: '2-digit',
    ...
    ? [
        {
          ...
          value: health.opportunities + health.draftActions,
          icon: Zap,
        ...
        {
          label: 'Bağlı Platformlar',
          value: health.connectedAccounts.platforms.join(', ') || '—',
          icon: Shield,
          color: 'text-primary',
    ...
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-8 border border-gray-700/50">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {stats.map((stat) => {
              ...
              return (
                <div
                  key={stat.label}
                  className="flex items-center gap-3 bg-white/[0.04] border border-white/[0.06] rounded-xl px-4 py-3 backdrop-blur-sm hover:bg-white/[0.07] transition-colors"
                >
      ...
      <div className="absolute top-3 right-3 z-20">
        <span className={`text-[10px] font-medium rounded px-1.5 py-0.5 ${
          ...
          {loading ? 'LOADING...' : aiGenerated ? 'AI ANALYSIS' : 'NO DATA'}
        </span>
      </div>
    </div>
  )
