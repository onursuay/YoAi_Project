...
/* SEVİYE 1 — Kampanya kartı (Faz 3).
   En üstte kampanya türü uyumsuzluğu uyarısı (varsa), sonra gerekçe +
   öneriler. Tıklama → SEVİYE 2 (ad set'ler). Tüm detaylar AÇIK. */
  ...
  return (
    <div className="relative text-left w-full rounded-2xl overflow-hidden border bg-[#0f172a] border-[#23314d] shadow-md flex flex-col h-full transition-all duration-200 hover:border-emerald-400/40">
      ...
      <div className="px-4 pb-1 relative">
        <p className="text-[9px] text-slate-400 uppercase tracking-wider">{t('campaignLevel')}</p>
        <p className="text-sm text-slate-100 font-semibold leading-snug">{campaign.campaign_name || '—'}</p>
        <p className="text-[10px] text-slate-400 mt-0.5">{t('currentType')}: <span className="text-slate-300">{currentType}</span></p>
      </div>

      {/* Kampanya türü uyumsuzluğu — büyük kırmızı uyarı, en üstte */}
      {mismatch && (
        <div className="mx-4 my-2 rounded-xl border border-red-500/50 bg-red-950/40 px-3 py-2.5 relative">
          <div className="flex items-center gap-1.5 mb-1">
            <AlertOctagon className="w-4 h-4 text-red-400" />
            <p className="text-[11px] font-bold text-red-300 uppercase tracking-wide">{t('typeMismatchTitle')}</p>
          </div>
          <p className="text-[11px] text-red-100/90 leading-relaxed">{mismatch.reason}</p>
          {mismatch.recommended_type ? (
            <p className="text-[10px] text-red-200 mt-1.5">{t('recommendedType')}: <span className="font-semibold">{mismatch.recommended_type}</span></p>
          ) : null}
          {mismatch.recommended_action ? (
            <p className="text-[10px] text-white font-semibold mt-1 bg-red-600/40 inline-block px-2 py-0.5 rounded">{mismatch.recommended_action}</p>
          ) : null}
        </div>
      )}
      ...
      {campaign.reasoning ? (
        <div className="mx-4 mb-2 relative">
          ...
          <p className="text-[11px] text-slate-300 leading-relaxed">{campaign.reasoning}</p>
        </div>
      ) : null}
