    ...
    <div className="border-t border-slate-700/40 mt-auto">
      ...
      {status === 'rejected_by_user' && (
        <div className="flex items-center justify-between py-2.5 px-3">
          <span className="text-[11px] text-slate-500">{t('status.rejected_by_user')}</span>
          <button onClick={onUndo} disabled={busy} className="text-[11px] text-emerald-400 hover:text-emerald-300 underline disabled:opacity-40">
            {t('undo')}
          </button>
        </div>
      )}
