...
import type { AdProposal, WinningPattern } from '@/lib/yoai/adCreator'
...
export default function AdCreationWizard({ onClose, connectedPlatforms }: Props) {
  ...
  const [patterns, setPatterns] = useState<WinningPattern | null>(null)
  ...
  const handleSelectPlatform = async (p: Platform) => {
    ...
    try {
      ...
      if (json.ok && json.data?.proposals?.length > 0) {
        ...
        setPatterns(json.data.patterns || null)
        setStep('preview')
      } else {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setPublishResult({
        ok: json.ok,
        message: json.message || json.error || 'İşlem tamamlandı',
      })
      setStep('done')
    } catch {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Mevcut performans verilerine dayalı AI reklam önerisi</p>
            </div>
          </div>
        ...
        <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {connectedPlatforms.includes('Meta') && (
                  <button
                    ...
                    className="p-6 rounded-2xl border-2 border-gray-200 hover:border-blue-300 hover:bg-blue-50/50 transition-all text-left group"
                  >
                    ...
                    <p className="text-xs text-gray-500">Facebook & Instagram reklamı</p>
                  </button>
                )}
                {connectedPlatforms.includes('Google') && (
                  <button
                    onClick={() => handleSelectPlatform('Google')}
                    className="p-6 rounded-2xl border-2 border-gray-200 hover:border-red-300 hover:bg-red-50/50 transition-all text-left group"
                  >
                    ...
                    <p className="text-xs text-gray-500">Search kampanyası (RSA)</p>
                  </button>
                )}
              </div>
              {connectedPlatforms.length === 0 && (
                <p className="text-sm text-gray-500 text-center py-8">Bağlı platform bulunamadı. Meta veya Google Ads hesabınızı bağlayın.</p>
              )}
            </div>
          )}
          ...
          {step === 'generating' && (
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">AI Reklam Üretiyor</h3>
              <p className="text-sm text-gray-500">Mevcut kampanyalar analiz ediliyor ve reklam önerileri oluşturuluyor...</p>
            </div>
          )}
          ...
          {step === 'preview' && proposals.length > 0 && (
            <div>
              ...
              {patterns && patterns.topCampaigns.length > 0 && (
                <div className="bg-gray-50 rounded-xl px-4 py-3 mb-5">
                  <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Kazanan Kalıplar</p>
                  <div className="flex flex-wrap gap-3 text-xs text-gray-600">
                    <span>Ort. CTR: <strong className="text-gray-900">{patterns.avgMetrics.ctr.toFixed(2)}%</strong></span>
                    <span>Ort. CPC: <strong className="text-gray-900">₺{patterns.avgMetrics.cpc.toFixed(2)}</strong></span>
                    {patterns.avgMetrics.roas != null && (
                      <span>Ort. ROAS: <strong className="text-gray-900">{patterns.avgMetrics.roas.toFixed(2)}x</strong></span>
                    )}
                  </div>
                </div>
              )}
              ...
              <h3 className="text-sm font-semibold text-gray-900 mb-4">
                {proposals.length} Reklam Varyasyonu — Birini Seçin
              </h3>
          ...
          {step === 'publishing' && (
            <div className="flex flex-col items-center justify-center py-16">
              <Loader2 className="w-10 h-10 text-primary animate-spin mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Reklam Oluşturuluyor</h3>
              <p className="text-sm text-gray-500">{platform} üzerinde reklam oluşturuluyor...</p>
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center py-12">
              {publishResult.ok ? (
                <>
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">Reklam Oluşturuldu</h3>
                  <p className="text-sm text-gray-600 text-center max-w-sm">{publishResult.message}</p>
                  <p className="text-xs text-gray-400 mt-2">Reklam PAUSED durumunda oluşturuldu. Aktif etmek için ilgili platform sayfasına gidin.</p>
                </>
              ) : (
                <>
              ...
              <button onClick={onClose} className="mt-6 px-6 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-200 transition-colors">
                Kapat
              </button>
            </div>
          )}
