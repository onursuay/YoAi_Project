...
export default function ErrorPanel({ error, onRetry, retrying }: ErrorPanelProps) {
  const t = useTranslations('dashboard.strateji.error')
    ...
    <div className="bg-red-50 border border-red-200 rounded-xl p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold text-red-800">{t('title')}</h4>
          <p className="text-sm text-red-700 mt-1">{error.message}</p>
          <div className="flex items-center gap-4 mt-2 text-xs text-red-500">
            <span>{t('code', { code: error.code })}</span>
            {error.timestamp && (
          ...
          {error.details && (
            <details className="mt-2">
              <summary className="text-xs text-red-500 cursor-pointer hover:text-red-700">{t('technicalDetail')}</summary>
