  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <p className="text-sm text-gray-500 mb-6">{g('company')}</p>
        ...
        <div className="bg-white rounded-lg p-8 space-y-6">
          ...
          <section>
            ...
            <div className="space-y-3">
              <p className="text-gray-700"><strong>{locale === 'tr' ? 'Zorunlu Çerezler:' : 'Essential Cookies:'}</strong> {g('section2Essential').replace(/^[^:]+:\s*/, '')}</p>
              <p className="text-gray-700"><strong>{locale === 'tr' ? 'İşlevsel Çerezler:' : 'Functional Cookies:'}</strong> {g('section2Functional').replace(/^[^:]+:\s*/, '')}</p>
              <p className="text-gray-700"><strong>{locale === 'tr' ? 'Analitik Çerezler:' : 'Analytics Cookies:'}</strong> {g('section2Analytics').replace(/^[^:]+:\s*/, '')}</p>
              <p className="text-gray-700"><strong>{locale === 'tr' ? 'Üçüncü Taraf Çerezler:' : 'Third-Party Cookies:'}</strong> {g('section2ThirdParty').replace(/^[^:]+:\s*/, '')}</p>
            </div>
          </section>
