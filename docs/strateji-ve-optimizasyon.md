...
3. 🟡 **Google Ads kanadı — Faz 1 eklendi** (2026-05-22) — Google için skor + tarama + advisory öneriler + UI (Meta/Google kaynak seçici). `/api/google/optimization/score` + `/magic-scan`, `GoogleCampaignCard` + `GoogleScanResults`. Mevcut Google read/mutate helper'ları kullanıldı, Meta yolu değişmedi. **Faz 2 açık:** tek-tık canlı apply (Google mutate helper'ları hazır) + Google-only kullanıcı için sayfa girişi.
