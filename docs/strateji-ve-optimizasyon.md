...
3. ✅ **Google Ads kanadı — Faz 1 + 2 tamam** (2026-05-22) — Google için skor + tarama + öneriler + UI (Meta/Google kaynak seçici) **+ tek-tık canlı apply/rollback** (kampanya duraklat / bütçe kıs, REVIEW_REQUIRED açık onay). `/api/google/optimization/score` + `/magic-scan` + `/apply`, `GoogleCampaignCard` + `GoogleScanResults`. Mevcut Google read/mutate helper'ları kullanıldı, Meta yolu değişmedi. **Faz 3 açık:** Google-only kullanıcı için sayfa girişi (şu an Meta bağlantı kapısına bağlı).
