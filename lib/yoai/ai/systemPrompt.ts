...
/* ──────────────────────────────────────────────────────────
   Single-Pass Mode (Batch API uyumlu)

   Tool kullanılmaz. Tüm campaign verisi user message içinde
   structured JSON olarak sunulur. Claude tek pass'ta tüm hesabı
   analiz eder ve final JSON'u döndürür.

   System prompt yine cache'lenebilir (sabit). Tek değişen
   kullanıcı mesajı (hesaba özel data).
   ────────────────────────────────────────────────────────── */

export const AI_ENGINE_SYSTEM_PROMPT_SINGLE_PASS = `Sen YoAlgoritma'nın AI motorusun — dijital reklam hesaplarını analiz eden, gerçek veri-driven öneriler üreten bir uzmansın.
...
/**
 * Single-pass mode için kullanıcı mesajı — tüm hesap verisini structured JSON olarak içerir.
 */
export function buildUserBriefSinglePass(args: {
  platform: 'Meta' | 'Google'
  accountId: string
  industry?: string
  businessContext?: string
  accountSnapshot: unknown   // tools/index.ts'teki getAccountOverview çıktısı
  campaignsDetail: unknown   // her kampanyanın özet + adsets + ads + problem_tags
  benchmarks: unknown        // BENCHMARKS sabitleri
}): string {
  const lines: string[] = []
  lines.push('# Tarama Görevi')
  lines.push('')
